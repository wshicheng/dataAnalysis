...
export default {
      ...
      columns_orderData: [
        {
          ...
          render: (h, params) => {
            if (that.cityType === 1) {
              ...
              return h("p", {}, params.row.orderTime);
            }
          }
        },
        {
        ...
        {
          renderHeader: h => {
            return h("div", [h("div", "订单金额"), h("span", "(￥)")]);
          },
          key: "orderAllAmount"
          // sortable: true
        },
        ...
        {
          renderHeader: h => {
            return h("div", [h("div", "平均订单时长"), h("span", "(min)")]);
          },
          key: "avgTime",
          width: 100
        },
        {
          renderHeader: h => {
            return h("div", [h("div", "平均订单里程"), h("span", "(km)")]);
          },
          key: "avgMileage",
          width: 100
        }
  ...
  methods: {
    ...
    loadData(type) {
      ...
      this.axios
        .get("/beefly/dateCityOrders/api/v1/wholeDataPage", {
          params: {
        ...
        .then(res => {
          ...
          if (res.data.resultCode != 1) {
            ...
            this.orderData = data;
            this.loadChartData(
              $(".orderAllData_head_time button.active").attr("myId")
            );
            this.loadTotalData(
              $(".orderAllData_head_time button.active").attr("myId")
            );
    ...
    loadChartData(type) {
      this.axios
        .get("/beefly/dateCityOrders/api/v1/chartData", {
          params: {
            accessToken: this.$store.state.token,
            type: type,
            cityCode: this.$store.state.cityList.toString(),
        ...
        .then(res => {
          ...
          if (res.data.resultCode === 0) {
            ...
            this.noDataBox = true;
            chartData.map(item => {
              this.chartDataPayAmount.push(
                Number(this.delcommafy(item.payAmount))
              );
              this.chartDisCountAmount.push(
                Number(this.delcommafy(item.disCountAmount))
              );
              this.chartProfitRate.push(Number(item.profitRate));
              this.chartDataX.push(
                this.cityType === 1 ? item.cityName : item.orderTime
              );
            });
    ...
    loadTotalData(type) {
      this.axios
        .get("/beefly/dateCityOrders/api/v1/wholeTotal", {
          params: {
            accessToken: this.$store.state.token,
            type: type,
            cityCode: this.$store.state.cityList.toString(),
