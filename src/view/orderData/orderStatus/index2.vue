...
<script>
  ...
  export default {
    ...
    data() {
      return {
          statusType:'close',// 对比状态下，图表类型
          initCityName:[],//自己定义的诚市名数组
    ...
    methods: {
      ...
      loadData(type, data_type, state, pageNo, pageSize) {
        ...
        this.axios
          ...
          .then(res => {
            // console.log(res);
            let arr1 = this.$store.state.keepCitys;
            // console.log(arr1);
            let arr2= [];
            for(let i=0;i<arr1.length;i++){
              arr2.push(arr1[i].name);
            };
            this.initCityName = arr2;
            console.log(this.initCityName);
            this.spinShow = false;
            this.noDataText = "暂无数据";
            // 判断是否超时
            this.checkLogin(res);
      ...
      orderComparisonData: {
        handler: function(val, old) {
          ...
          var closeNum = data.map(item => {
            var tem = []
            tem.push([item.cityName,1 * item.closeNum])
            return tem;
          });
          console.log('人工关闭的数据-----',closeNum)
          // 开锁失败的订单数的数据failNum
          var failNum = data.map(item => {
            var tem = item.failNum;
            return {
              name: item.cityName,
              data: tem
            };
