<template>
  <div id="activeUser_body">
        <Breadcrumb class="Breadcrumb">
            <BreadcrumbItem>活跃用户</BreadcrumbItem>
        </Breadcrumb>
      <div id="activeUser_head">
        <div class="activeUser_head_time">
            <span>时间:</span>
      ...
      <div class="activeUser_table">
        ...
        <div class="spieceTablePar">
             <Table  class="spieceTable"  border size='small' :no-data-text='noDataTextSpace' :columns="columns_activeUserData" :data="[]"></Table>
        </div>
        <div class="relTab">
             <Table  border size='small' :no-data-text='noDataText' :columns="columns_activeUserData" :data="activeUserData"></Table>
        </div>
      ...
      <div class="activeUser_chart" v-show="noDataBox">
...
export default {
                    ...
                    render: (h, params) => {
                        if (that.cityType === 1) {
                            return h('a', {
                                ...
                                attrs: {
                                    target: '_blank',
                                    href: '#/index/activeUserDetail/detail/' + params.row.cityCode + '&' + params.row.cityName
                                }
                            }, params.row.cityName)
                        } else {
                            return h('p', {
    ...
    mounted () {
        ...
        this.$store.dispatch('menuActiveName', '/index/activeUser')
               ...
               if(-(_relTop-partop) > _selfHeight){
                //    console.log('此时假头部改出现了')
                   if(Math.abs(-(top-partop) - _selfHeight)<10){
                    ...
                    $('div.spieceTable').parent().css({
                        position:'absolute',
                        top:Math.abs(-(_relTop-partop)) + _selfHeight ,
                        zIndex:99
                    }) 
               }else{
                    $('div.spieceTable').parent().css({
